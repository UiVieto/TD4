...
Livre* creerLivre(istream& nomFichier)
{
	string auteur, titre;
	int aneeSortie, ventes, nbPages;

	nomFichier >> quoted(titre);
	nomFichier >> aneeSortie;
	nomFichier >> quoted(auteur);
	nomFichier >> ventes;
	nomFichier >> nbPages;
	Livre* livre = new Livre(auteur, titre, aneeSortie, ventes, nbPages);
	cout << *livre;
	return livre;
}

void ajouterLivres(string nomfichier, vector<Item*>& bibliotheque) {

	ifstream fichier(nomfichier);

	while (!fichier.eof()) { 
		bibliotheque.push_back(creerLivre(fichier));
		cout << " AjoutLivre";
	}
...
void afficherListeItem(vector<Item*>& bibliotheque)
{
...
int main()
{
	...
	//ajoutLivre
	cout << "ajoutlivre";
	ajouterLivres("livres.txt", biblio);

	//Affichage biblio
	afficherListeItem(biblio);
